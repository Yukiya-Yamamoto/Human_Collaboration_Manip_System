...
##################################################################
  ...
  with work_detection:
    StateMachine.add('PICK MOTION 1', TF_MANIP(0.03, 0.015, 0.20, tf_pose="target_frame"), \
...
#周辺環境計測
    StateMachine.add('AREA SET',  AREA_SET(set_data), \
        transitions={'succeeded':'WORK DETECTION 1','retry':'AREA SET','aborted':'aborted'})
#ワーク検知１    
...
#待機中動作
  ...
  with preparation:  
    StateMachine.add('SYSTEM CALL',  SYSTEM_CALL(), \
        transitions={'succeeded':'succeeded','retry':'AREA SET','aborted':'aborted'})
